    ...
    def centralized_train(self, agents, replay_buffer, iterations, batch_size, discount, tau, 
        ...
        for it in range(iterations):
            ...
            x_n, y_n, u_n, r_n, d_n = replay_buffer.centralized_sample(
                batch_size=batch_size, n_agent=len(agents))  # NOTE It returns a list with exp for all agents

            state_n = [
                torch.FloatTensor(x_n[i_agent_]).to(device)
                for i_agent_ in range(len(agents))]
            action_n = [
            ...
            for i_agent_ in range(len(agents)):
                with torch.no_grad():
                    ...
                    next_action = agents[i_agent_].policy.actor_target(next_state_n[i_agent_]) + noise
                    next_action = next_action.clamp(-self.max_action, self.max_action)
                ...
                for i_agent_ in range(len(agents)):
                    if i_agent_ == self.i_agent:
                        with torch.enable_grad():
                            action = agents[i_agent_].policy.actor(state_n[i_agent_])
